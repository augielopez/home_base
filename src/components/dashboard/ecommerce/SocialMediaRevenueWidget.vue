...
const bgColors = ['#0EA5E9', '#EAB308', '#A855F7', '#F97316'];
const dataLabels = [
...
onBeforeMount(() => {
    randomData.value = generateRandomMultiData('2020-10-27T00:00:00', '2023-11-03T00:00:00', 4, 100, 700, 4);
});
</script>
        ...
        <div class="mt-6 flex flex-col">
            <template v-for="(item, index) of dataLabels" :key="index">
                <div class="flex items-center gap-3.5">
                    <span class="w-2 h-2 rounded-full" :style="{ background: bgColors?.[index] }" />
                    ...
                    <span class="label-small text-surface-950 dark:text-surface-0">${{ computedData?.[index].toFixed(2) }}</span>
                </div>
