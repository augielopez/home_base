...
<template>
    <div class="p-6 card">
        <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-7 mb-7">
            <div class="flex flex-col sm:flex-row justify-end items-stretch sm:items-center gap-4 w-full ml-auto">
                ...
                <div class="w-full sm:w-[148px]">
                    <Select v-model="selectedCategory" :options="categories" optionLabel="label" optionValue="value" placeholder="Category" class="w-full rounded-xl border border-surface-200 dark:border-surface-700" />
                </div>

                <div class="w-full sm:w-[148px]">
                    <Select v-model="selectedSort" :options="sortOptions" optionLabel="label" optionValue="value" placeholder="Sort By" class="w-full rounded-xl border border-surface-200 dark:border-surface-700" />
                </div>
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            <div v-for="product in filteredAndSortedProducts" :key="product.id" class="h-120 p-4 rounded-3xl flex flex-col justify-end items-start gap-2 overflow-hidden relative group cursor-pointer">
                ...
                <div class="relative z-10 w-full p-4 bg-surface-0 dark:bg-surface-900 rounded-2xl shadow-xl flex flex-col justify-start items-start gap-2 overflow-hidden">
                    <div class="w-full flex justify-between items-center gap-2">
                        <Tag :value="getCategoryLabel(product.category)" severity="secondary" class="text-xs font-medium" />
        ...
        <div v-if="filteredAndSortedProducts.length === 0" class="w-full text-center py-12">
