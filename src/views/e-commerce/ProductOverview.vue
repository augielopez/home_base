...
const filteredReviews = computed(() => {
    if (selectedRating.value === 'all') {
        const allReviews = Object.values(reviews.value).flat();
        return allReviews;
    }
    const ratingReviews = reviews.value[selectedRating.value] || [];
...
function setSize(size) {
    if (!sizes.value.find((s) => s.label === size)?.disabled) {
...
const toggleReviewMenu = (event, reviewIndex) => {
    const menuKey = `reviewMenu_${reviewIndex}`;
    const menu = reviewMenus.value[menuKey];
    if (menu) {
        menu.toggle(event);
    }
};
    ...
    <div class="md:p-6 card">
        <div class="flex flex-col xl:flex-row justify-start items-start gap-6">
            <div class="w-full xl:flex-1 grid grid-cols-1 sm:grid-cols-2 gap-6">
                <img class="w-full aspect-square object-cover rounded-3xl border border-surface-200 dark:border-surface-700" :src="images[0]" alt="Product image 1" />
                <img class="w-full aspect-square object-cover rounded-3xl border border-surface-200 dark:border-surface-700" :src="images[1]" alt="Product image 2" />
                <img class="w-full aspect-square object-cover rounded-3xl border border-surface-200 dark:border-surface-700" :src="images[2]" alt="Product image 3" />
                <img class="w-full aspect-square object-cover rounded-3xl border border-surface-200 dark:border-surface-700" :src="images[3]" alt="Product image 4" />
            </div>
            ...
            <div class="w-full xl:w-96 shrink-0 flex flex-col justify-start items-start gap-6">
                <div class="w-full bg-surface-0 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl overflow-hidden">
                    <div class="p-6 border-b border-surface-200 dark:border-surface-700 flex flex-col justify-start items-start gap-4">
                        <Tag value="High Score" severity="warning" class="bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-200" />
                    ...
                    <div class="p-6 border-b border-surface-200 dark:border-surface-700 flex flex-col justify-start items-start gap-4">
                        ...
                        <div class="w-full flex justify-start items-center gap-2">
                            <div
                                v-for="size in sizes"
                                :key="size.label"
                                class="flex-1 h-8 p-1 rounded-xl transition-colors border"
                                :class="{
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div v-if="size.disabled" class="absolute inset-0 flex items-center justify-center">
                                        <div class="w-6 h-0.5 bg-surface-500 dark:bg-surface-400" />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="p-6 flex justify-start items-center gap-2">
                        <Button label="Add to cart" class="flex-1 self-stretch" />
                        <Button icon="pi pi-heart" outlined severity="secondary" class="self-stretch" />
                        <Button icon="pi pi-share-alt" outlined severity="secondary" class="self-stretch" />
                    </div>
                </div>
                ...
                <div class="w-full bg-surface-0 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl overflow-hidden">
                    ...
                    <div class="px-6 py-4">
                        <Button label="Track the cargo" severity="secondary" outlined class="w-full rounded-full" />
                    </div>
                </div>
                ...
                <div class="w-full p-6 bg-surface-50 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <Button label="Explore" link class="p-2 text-primary-600 dark:text-primary-400" iconPos="right" icon="pi pi-arrow-right" />
                    </div>
        ...
        <div class="w-full bg-surface-0 dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-3xl overflow-hidden mt-6">
            <Tabs v-model:value="activeTab">
                <TabList class="border-b border-surface-200 dark:border-surface-700">
                    <Tab value="description" class="flex-1 p-4 text-center">
                        <span
                    ...
                    <Tab value="evaluations" class="flex-1 p-4 text-center">
                        <span
                    ...
                    <Tab value="qa" class="flex-1 p-4 text-center">
                        <span
                            class="text-base font-normal"
                            :class="{
                ...
                <TabPanels>
                    <TabPanel value="description" class="p-6">
                    ...
                    <TabPanel value="evaluations" class="p-0">
                        <div class="w-full flex flex-col">
                                ...
                                <div class="w-full xl:w-80 flex flex-col border-b border-surface-200 dark:border-surface-700">
                                    ...
                                    <div class="border-t border-surface-200 dark:border-surface-700 flex min-h-[36px]">
                                        <div class="flex-1 p-2 flex justify-center items-center gap-2">
                                            <div class="w-3 h-3 bg-yellow-500 rounded-sm border border-surface-0 dark:border-surface-800" />
                                            <div class="text-center text-surface-900 dark:text-surface-0 text-base font-medium leading-normal">Positive (83%)</div>
                                        </div>
                                        <div class="w-px bg-surface-200 dark:bg-surface-700" />
                                        <div class="flex-1 p-2 flex justify-center items-center gap-2">
                                            <div class="w-3 h-3 bg-surface-200 dark:bg-surface-700 rounded-sm border border-surface-0 dark:border-surface-800" />
                                        ...
                                        <div class="flex-1 flex flex-col justify-between gap-4">
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 h-1 bg-surface-200 dark:bg-surface-700 rounded-full relative">
                                                    <div class="w-4/5 h-full bg-yellow-500 rounded-full" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 h-1 bg-surface-200 dark:bg-surface-700 rounded-full relative">
                                                    <div class="w-3/5 h-full bg-yellow-500 rounded-full" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 h-1 bg-surface-200 dark:bg-surface-700 rounded-full relative">
                                                    <div class="w-2/5 h-full bg-yellow-500 rounded-full" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 h-1 bg-surface-200 dark:bg-surface-700 rounded-full relative">
                                                    <div class="w-1/5 h-full bg-yellow-500 rounded-full" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div class="flex-1 h-1 bg-surface-200 dark:bg-surface-700 rounded-full relative">
                                                    <div class="w-1/12 h-full bg-yellow-500 rounded-full" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="w-full">
                                <div v-if="selectedRating !== 'all'" class="px-6 pt-4 pb-2">
                                    <Button label="Show All Reviews" text @click="selectRating('all')" class="text-primary-600 dark:text-primary-400 hover:bg-surface-100 dark:hover:bg-surface-800" />
                                </div>

                                <div v-for="(review, index) in filteredReviews" :key="`${review.rating}-${index}`" class="px-6 pt-6 pb-8" :class="{ 'border-b border-surface-200 dark:border-surface-700': index !== filteredReviews.length - 1 }">
                                            ...
                                            <div class="flex items-center gap-2 mb-3">
                                                <div class="flex-1 flex flex-col md:flex-row md:items-center gap-2 min-w-0">
                                                    ...
                                                    <div class="flex items-center gap-2 md:gap-3 md:ml-auto shrink-0">
                                                        <div class="flex items-center gap-2">
                                                            <div class="text-surface-500 dark:text-surface-400 text-sm md:text-base font-normal leading-normal whitespace-nowrap">{{ review.date.split(' | ')[0] }}</div>
                                                            <div class="hidden sm:block w-px h-2 bg-surface-200 dark:bg-surface-700" />
                                                            <div class="text-surface-500 dark:text-surface-400 text-sm md:text-base font-normal leading-normal whitespace-nowrap">{{ review.date.split(' | ')[1] }}</div>
                                                        </div>
                                                        <div class="hidden md:block w-px h-2 bg-surface-200 dark:bg-surface-700" />
                                                        <div class="flex items-center gap-2">
                                                ...
                                                <div class="relative shrink-0">
                                                    <Button icon="pi pi-ellipsis-v" text size="small" severity="secondary" @click="toggleReviewMenu($event, index)" class="p-2! h-auto!" />
                                                    <Menu
                                                        :ref="
                                                            (el) => {
                                                                if (el) reviewMenus[`reviewMenu_${index}`] = el;
                                                            }
                                                        "
                    ...
                    <TabPanel value="qa" class="p-6">
                        <div class="flex flex-col gap-7">
                            <template v-for="(qa, index) in qaItems" :key="index">
