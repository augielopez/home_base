...
<template>
    <div class="p-6 card">
        <div class="flex flex-col xl:flex-row justify-between items-start gap-6 mb-6">
            ...
            <div class="w-full xl:w-[217px]">
                <IconField iconPosition="left">
                    <InputIcon class="pi pi-search text-surface-500 dark:text-surface-400" />
        ...
        <div class="flex flex-col gap-6">
            <div v-for="order in filteredOrders" :key="`order-wrapper-${order.id}`" class="rounded-3xl border border-surface-200 dark:border-surface-700 overflow-hidden">
                <div class="p-6 xl:p-8 relative cursor-pointer hover:bg-surface-50 dark:hover:bg-surface-800 transition-colors" @click="toggleOrder(order.id)">
                    <div class="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 xl:gap-6 pr-8">
                        <div class="flex items-center gap-4 xl:gap-8 w-full xl:w-auto">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 flex-1 xl:flex-none">
                                ...
                                <div class="block sm:hidden">
                                    <Tag :value="order.statusLabel" :severity="order.statusColor" class="text-xs font-semibold whitespace-nowrap" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-6 xl:gap-8 w-full xl:w-auto">
                            <div class="hidden sm:block xl:w-40">
                                <Tag :value="order.statusLabel" :severity="order.statusColor" class="text-sm font-semibold whitespace-nowrap" />
                            </div>
                    ...
                    <i
                        :class="[
                            'pi pi-chevron-down',
                            expandedOrders[order.id] ? 'rotate-180' : 'rotate-0',
                            'text-surface-500  transition-transform duration-200 dark:text-surface-400 text-lg xl:text-xl absolute right-6 xl:right-8 top-1/2 -translate-y-1/2 pointer-events-none'
                        ]"
                    />
                </div>
                            ...
                            <div class="p-7">
                                <div class="grid grid-cols-1 xl:grid-cols-2 gap-8 mb-7">
                                    <div class="flex items-start gap-4">
                                        <img class="w-[116px] h-[116px] rounded-xl" :src="order.image" :alt="order.product" />
                                        <div class="flex flex-col gap-3">
                                            <div class="flex items-center gap-3">
                                                <span class="text-surface-500 dark:text-surface-400 text-sm font-medium">Seller</span>
                                                <div class="flex items-center gap-1">
                                                    <i class="pi pi-verified text-surface-950 dark:text-surface-0 text-base" />
                                ...
                                <div class="h-px bg-surface-200 dark:bg-surface-700 my-6" />

                                <div class="grid grid-cols-1 xl:grid-cols-2 gap-8">
                                    <div class="flex flex-col gap-6">
                                        <div class="relative h-4">
                                            <div class="absolute inset-0 flex items-center">
                                                <div class="h-0.5 w-full bg-surface-200 dark:bg-surface-700" />
                                            </div>
                                            <div class="absolute inset-0 flex items-center">
                                                <div
                                                    :class="[
                                                        'h-0.5 transition-all duration-500 bg-primary',
                                            ...
                                            <div class="relative flex justify-between">
                                                <div
                                                    :class="[
                                                        'w-4 h-4 rounded-full border-2 bg-white dark:bg-surface-900 transition-colors',
                                                        getDeliveryProgress(order.deliveryStatus) >= 1 ? 'border-primary' : 'border-surface-200 dark:border-surface-700'
                                                    ]"
                                                />
                                                <div
                                                    :class="[
                                                        'w-4 h-4 rounded-full border-2 bg-white dark:bg-surface-900 transition-colors',
                                                        getDeliveryProgress(order.deliveryStatus) >= 2 ? 'border-primary' : 'border-surface-200 dark:border-surface-700'
                                                    ]"
                                                />
                                                <div
                                                    :class="[
                                                        'w-4 h-4 rounded-full border-2 bg-white dark:bg-surface-900 transition-colors',
                                                        getDeliveryProgress(order.deliveryStatus) >= 3 ? 'border-primary' : 'border-surface-200 dark:border-surface-700'
                                                    ]"
                                                />
                                                <div
                                                    :class="[
                                                        'w-4 h-4 rounded-full border-2 bg-white dark:bg-surface-900 transition-colors',
                                                        getDeliveryProgress(order.deliveryStatus) >= 4 ? 'border-primary' : 'border-surface-200 dark:border-surface-700'
                                                    ]"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex flex-col gap-2">
                                            <div class="flex items-center justify-between">
                                                ...
                                                <Button label="Cargo tracking" icon="pi pi-map-marker" link class="p-0" />
                                            </div>
                                            <div>
