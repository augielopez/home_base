...
const subtotal = 1001.47;
const totalSavings = 111.5;
const shippingOriginal = 18.0;
const vat = 20.0;
const total = 909.97;
...
<template>
    <div class="bg-surface-0 dark:bg-surface-900 relative card">
        <div class="bg-surface-50 dark:bg-surface-800 border-b border-surface-200 dark:border-surface-700">
            <div class="max-w-7xl mx-auto p-6 xl:px-8">
                <div class="flex items-center gap-3 mb-8">
                    <i class="pi pi-arrow-left text-surface-950 dark:text-surface-0 cursor-pointer hover:text-surface-700 dark:hover:text-surface-300" />
                    <div class="text-surface-950 dark:text-surface-0 text-xl font-medium">Checkout Form</div>
                </div>
        ...
        <div class="max-w-7xl mx-auto px-6 xl:px-8 relative">
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                <div class="xl:col-span-2 space-y-8 pt-12">
                    <div class="space-y-6">
                        <div v-for="item in cartItems" :key="item.id" class="flex items-start gap-4">
                            ...
                            <div class="flex-1 flex flex-col gap-3">
                                <div class="flex justify-between items-start">
                                    <div class="text-surface-950 dark:text-surface-0 text-lg font-medium">{{ item.name }}</div>
                                    <div class="text-surface-950 dark:text-surface-0 text-base font-medium">${{ item.price.toFixed(2) }}</div>
                                </div>

                                <div class="w-full border-b border-dashed border-surface-200 dark:border-surface-600" />
                ...
                <div class="xl:col-span-1">
                    <div class="bg-white dark:bg-surface-800 rounded-3xl shadow-xl p-8 space-y-6 xl:-mt-40 min-w-[340px]">
                        <div class="h-44 relative bg-linear-to-br from-primary to-primary-400/80 rounded-2xl overflow-hidden backdrop-blur-sm">
                        ...
                        <div class="flex flex-col gap-2">
                            <label class="text-surface-900 dark:text-surface-0 text-base">Email</label>
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col gap-3 w-full">
                                <InputMask v-model="formData.cardNumber" mask="9999 9999 9999 9999" placeholder="1234 1234 1234 1234" class="w-full" />
                                <div class="flex gap-3 w-full">
                                    <InputMask v-model="formData.expiryDate" mask="99/99" placeholder="MM/YY" class="flex-1 w-full" />
                                    <InputMask v-model="formData.cvv" mask="999?9" placeholder="CVV" class="w-36 xl:w-24" />
                                </div>
                            </div>
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="text-surface-900 dark:text-surface-0 text-base">Cardholder Name</label>
                            <InputText v-model="formData.cardholderName" class="w-full" />
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="text-surface-900 dark:text-surface-0 text-base">Country</label>
                            <Select v-model="formData.country" :options="countries" optionLabel="label" optionValue="value" placeholder="United States" class="w-full" />
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="text-surface-900 dark:text-surface-0 text-base">Tax ID number</label>
                            <InputText v-model="formData.taxId" class="w-full" />
                        </div>

                        <div class="flex flex-col gap-2">
                            <label class="text-surface-900 dark:text-surface-0 text-base">Discount code</label>
                            <InputText v-model="formData.discountCode" class="w-full" />
                        </div>

                        <Button :label="`Pay $${total.toFixed(2)}`" class="w-full px-3 py-2 rounded-xl text-base font-medium" @click="processPayment" />
                    </div>
                </div>
            </div>
        </div>
