...
const totalEarnings = computed(() => {
    return cartItems.value.reduce((sum, item) => sum + item.earnings * item.quantity, 0);
});

const vat = computed(() => 20.0);
...
<template>
    <div class="flex flex-col card">
        <div class="flex flex-col xl:flex-row">
                    ...
                    <div class="p-4 sm:p-6 bg-surface-0 dark:bg-surface-900 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                        ...
                        <div class="flex-1 w-full flex flex-col gap-3 sm:gap-4">
                            ...
                            <div class="flex items-center gap-2">
                                <i class="pi pi-clock text-surface-500 dark:text-surface-400" />
                                <div class="text-surface-500 dark:text-surface-400 text-base">Estimated {{ item.deliveryDate }}</div>
                            </div>
                        </div>

                        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                            <InputNumber :modelValue="item.quantity" @update:modelValue="(value) => updateQuantity(item.id, value)" showButtons buttonLayout="horizontal" :min="1" :max="10" inputClass="w-12!" />
                            <Button icon="pi pi-trash" severity="danger" outlined @click="removeItem(item.id)" />
                        </div>
                    </div>

                    <div v-if="index < cartItems.length - 1" class="px-6 py-2">
            ...
            <div class="w-full xl:w-96 bg-surface-50 dark:bg-surface-800 flex flex-col">
                <div class="p-6 flex flex-col gap-4">
                    ...
                    <Button label="Check Out" class="w-full px-3 py-2 rounded-xl text-base font-medium" @click="checkout" />
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-end gap-2">
                        <div class="flex-1 flex flex-col gap-2">
                            ...
                            <InputText v-model="promoCode" placeholder="Enter Coupon" class="w-full" />
                        </div>
                        <Button label="Apply" severity="contrast" class="w-full sm:w-auto" @click="applyPromoCode" />
                    </div>
                </div>
                ...
                <div class="flex-1 p-6 bg-surface-50 dark:bg-surface-800 flex flex-col gap-3 overflow-hidden">
                    <div class="flex justify-between items-center">
                        ...
                        <Button label="Change" link class="px-3 py-2 rounded-full" icon="pi pi-chevron-right" iconPos="right" />
                    </div>
        ...
        <div class="p-4 sm:p-6 border-t border-surface-200 dark:border-surface-700 flex flex-col gap-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
                <div v-for="product in recommendedProducts" :key="product.id" class="h-96 sm:h-120 p-4 rounded-3xl flex flex-col justify-end items-start gap-2 overflow-hidden relative group cursor-pointer">
                        ...
                        <div class="w-full flex flex-col gap-2">
                            <div class="w-full flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                                ...
                                <div class="flex justify-start items-center gap-2 ml-auto">
                                    <div class="p-2 rounded-lg border border-surface-200 dark:border-surface-700 flex justify-center items-center cursor-pointer hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors">
                                        <i class="pi pi-heart text-surface-500 dark:text-surface-400 text-sm sm:text-base" />
                                    </div>
                                    <div class="p-2 rounded-lg border border-surface-200 dark:border-surface-700 flex justify-center items-center cursor-pointer hover:bg-surface-100 dark:hover:bg-surface-700 transition-colors">
                                        <i class="pi pi-shopping-cart text-surface-500 dark:text-surface-400 text-sm sm:text-base" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="w-full flex justify-between items-center">
                                ...
                                <div class="px-2 py-1 bg-yellow-100 dark:bg-yellow-900 rounded-md flex justify-start items-center gap-1">
                                    <i class="pi pi-star-fill text-yellow-600 dark:text-yellow-400 text-xs" />
