    ...
    <div class="p-4 md:p-6 card">
        <div class="relative w-full h-80 md:h-96 lg:h-[525px] rounded-2xl md:rounded-3xl overflow-hidden" @mouseenter="stopAutoSlide" @mouseleave="startAutoSlide">
            <div class="relative w-full h-full">
                <div v-for="(slide, index) in slides" :key="slide.id" class="absolute inset-0 transition-opacity duration-1000 ease-in-out" :class="{ 'opacity-100': currentSlide === index, 'opacity-0': currentSlide !== index }">
                    <img :src="slide.image" :alt="slide.title" class="w-full h-full object-cover" />
                    <div class="absolute inset-0 bg-linear-to-b from-black/0 to-black/30 to-70%" />
                </div>
            </div>
            ...
            <div class="absolute w-full h-full flex flex-col justify-end gap-3 md:gap-4 top-0 left-0 p-4 md:p-6 lg:p-8">
                ...
                <div class="flex items-center gap-2 md:gap-3">
                    <div class="p-1 rounded-full border border-white/70 flex items-center overflow-hidden">
                        <img :src="slides[currentSlide].author.avatar" :alt="slides[currentSlide].author.name" class="w-6 h-6 md:w-8 md:h-8 rounded-full object-cover" />
                    </div>
            ...
            <div class="absolute right-0 bottom-0 bg-white/25 rounded-tl-2xl md:rounded-tl-3xl border border-r-0 border-b-0 border-white/30 flex items-center text-surface-0">
                <button @click="prevSlide" class="px-4 md:px-6 py-3 md:py-4 flex items-center justify-center hover:opacity-75 transition-opacity cursor-pointer" aria-label="Previous slide">
                    <i class="pi pi-arrow-left text-sm md:text-base pointer-events-none" />
                </button>

                <div class="w-px h-2 md:h-3 bg-white/25" />
                ...
                <button @click="nextSlide" class="px-4 md:px-6 py-3 md:py-4 flex items-center justify-center hover:opacity-75 transition-opacity cursor-pointer" aria-label="Next slide">
                    <i class="pi pi-arrow-right text-sm md:text-base pointer-events-none" />
                </button>
            </div>
        </div>
        ...
        <div class="mt-8 md:mt-12 grid grid-cols-12 gap-0">
            <div class="col-span-12 lg:col-span-4 xl:col-span-3 border-b lg:border-b-0 lg:border-r border-dashed border-surface-200 dark:border-surface-700 pr-0 lg:pr-6 pb-8 lg:pb-0">
                <div class="flex flex-col gap-6">
                    <template v-for="(post, index) in blogPosts[0]" :key="post.id">
                        <div class="flex flex-col gap-4">
                            <div class="flex flex-col gap-2">
                                ...
                                <div v-if="post.hasImage" class="h-48 md:h-64 bg-surface-100 dark:bg-surface-800 rounded-2xl md:rounded-3xl overflow-hidden">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <i class="pi pi-arrow-right text-primary-600 dark:text-primary-400 text-xs" />
                            </div>
                        </div>

                        <div v-if="index < blogPosts[0].length - 1" class="w-full h-0 border-t border-dashed border-surface-200 dark:border-surface-700" />
                    </template>
                </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-4 xl:col-span-6 border-b lg:border-b-0 lg:border-r border-dashed border-surface-200 dark:border-surface-700 px-0 lg:px-6 py-8 lg:py-0">
                <div class="flex flex-col gap-6">
                    <template v-for="(post, index) in blogPosts[1]" :key="post.id">
                        <div class="flex flex-col gap-6">
                            <div class="flex flex-col gap-2">
                                <div class="text-surface-500 dark:text-surface-400 text-base font-normal leading-normal">
                                    <span class="uppercase">{{ post.category }}</span> -
                            ...
                            <div v-if="post.hasImage" class="h-64 md:h-72 bg-surface-100 dark:bg-surface-800 rounded-2xl md:rounded-3xl overflow-hidden">
                                <img v-if="post.image" :src="post.image" :alt="post.title" class="w-full h-full object-cover" />
                            </div>
                            <p class="text-surface-500 dark:text-surface-400 text-base font-normal leading-normal">
                                {{ post.description }}
                        ...
                        <div v-if="index < blogPosts[1].length - 1" class="w-full h-0 border-t border-dashed border-surface-200 dark:border-surface-700" />
                    </template>
                </div>
            </div>
            ...
            <div class="col-span-12 lg:col-span-4 xl:col-span-3 pl-0 lg:pl-6 pt-8 lg:pt-0">
                <div class="flex flex-col gap-6">
                    <template v-for="(post, index) in blogPosts[2]" :key="post.id">
                        <div class="flex flex-col gap-4">
                            <div class="flex flex-col gap-2">
                                <div class="text-surface-500 dark:text-surface-400 text-base font-normal leading-normal">
                                    <span class="uppercase">{{ post.category }}</span> -
                        ...
                        <div v-if="index < blogPosts[2].length - 1" class="w-full h-0 border-t border-dashed border-surface-200 dark:border-surface-700" />
                    </template>
                </div>
            </div>
        </div>
